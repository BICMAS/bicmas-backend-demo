...
const TGI_COLORS = {
    ...
    primaryDark: rgb(1 / 255, 26 / 255, 80 / 255), // darkened #02236a
...
const THEME_STYLES = {
    ...
    tgi: {
        background: TGI_COLORS.white,
        primary: TGI_COLORS.text,
        accent: TGI_COLORS.primary,
        muted: TGI_COLORS.text,
        titleSize: 46,
        nameSize: 38,
        bodySize: 14,
        titleFont: 'HelveticaBold',
...
function isTgiTheme(themeName) {
    return themeName === 'tgi' || themeName === 'mecure';
}

function getThemeStyle(themeName) {
    if (isTgiTheme(themeName)) {
        return THEME_STYLES.tgi;
    }
...
async function drawTgiLogo(page, pdfDoc, template, footerTopY) {
    if (!template?.blobUrl) {
        return footerTopY;
    }
...
async function generateTgiCertificate({
    pdfDoc,
    page,
    template,
    themeConfig,
    ...
    drawCenteredText(page, 'CERTIFICATE', height - 88, styles.titleSize, titleFont, TGI_COLORS.secondary);
    ...
    drawCenteredText(page, traineeName, height - 210, nameSize, scriptFont, TGI_COLORS.secondary);
    drawCenteredText(
        page,
    ...
    await drawTgiLogo(page, pdfDoc, template, footerReservedHeight);
}
...
async function generateStandardCertificate({
    ...
    drawCenteredText(page, 'TGI Distri', contentTop, 11, bodyFont, headerColor);
    drawCenteredText(
        page,
...
export class CertificatePdfService {
    static async generateAndUpload({ template, traineeName, courseTitle, issuedAt }) {
        ...
        if (isTgiTheme(themeConfig.theme)) {
            await generateTgiCertificate({
                pdfDoc,
                page,
                template,
                themeConfig,
