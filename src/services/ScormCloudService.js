...
const DEFAULT_SCORM_REDIRECT_ON_EXIT_URL =
    'https://your-trainee-portal.example.com/scorm-exit.html';

export class ScormCloudService {
    static client = null;

    static getRedirectOnExitUrl(override) {
        const fromArg = typeof override === 'string' ? override.trim() : '';
        if (fromArg) return fromArg;

        const fromEnv = process.env.SCORM_REDIRECT_ON_EXIT_URL?.trim();
        if (fromEnv) return fromEnv;

        return DEFAULT_SCORM_REDIRECT_ON_EXIT_URL;
    }
    ...
    static async createLaunchLink(courseId, learnerId, learnerName, options = {}) {
        ...
        const launchPayload = {
            redirectOnExitUrl: ScormCloudService.getRedirectOnExitUrl(options.redirectOnExitUrl),
            launchAuth: {
                type: "vault",
                options: {
                    ipAddress: false,
